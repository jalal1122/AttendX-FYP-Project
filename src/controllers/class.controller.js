...
export const createClass = asyncHandler(async (req, res) => {
  ...
  if (!isUnique) {
    throw ApiError.internal('Failed to generate unique class code. Please try again');
  }
  ...
  res
    .status(201)
    .json(
      new ApiResponse(201, populatedClass, 'Class created successfully')
    );
});
...
export const joinClass = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json(
      new ApiResponse(200, updatedClass, 'Joined class successfully')
    );
});
