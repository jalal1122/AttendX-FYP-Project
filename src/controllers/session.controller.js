...
export const startSession = asyncHandler(async (req, res) => {
  ...
  if (classDoc.teacher.toString() !== req.user._id.toString()) {
    throw ApiError.forbidden('You are not authorized to start a session for this class');
  }
...
export const createRetroactiveSession = asyncHandler(async (req, res) => {
  ...
  if (!classId || !date || !startTime || !endTime) {
    throw ApiError.badRequest('Class ID, date, start time, and end time are required');
  }
  ...
  // Verify teacher owns this class
  if (classDoc.teacher.toString() !== req.user._id.toString()) {
    throw ApiError.forbidden('You are not authorized to create a session for this class');
  }
  ...
  if (sessionStartTime > new Date()) {
    throw ApiError.badRequest('Cannot create retroactive session for future date');
  }
