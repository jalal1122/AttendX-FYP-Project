 ...
 * GET /api/v1/analytics/student/students
 ...
 */
export const getStudentStudentReports = asyncHandler(async (req, res) => {
  ...
  if (isExport === "true") {
    ...
    const buffer = await ExportService.generateStudentReport("students", result, "xlsx", formattedDetails);
    res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
    res.setHeader("Content-Disposition", `attachment; filename=student_students_report.xlsx`);
  ...
  res.status(200).json(
    ...
    }, "Student student reports retrieved"),
  );
});

/**
 * Student Teacher Reports
 * GET /api/v1/analytics/student/teachers
 */
export const getStudentTeacherReports = asyncHandler(async (req, res) => {
  if (req.user.role !== "student") {
    throw ApiError.forbidden("Only students can access student reports");
  }
  ...
  if (isExport === "true") {
    ...
    const buffer = await ExportService.generateStudentReport("teachers", result, "xlsx", formattedDetails);
    res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
    res.setHeader("Content-Disposition", `attachment; filename=student_teachers_report.xlsx`);
    return res.status(200).send(buffer);
  }
  ...
  res.status(200).json(
    ...
    }, "Student teacher reports retrieved"),
  );
});

/**
 * Student Department Reports
 * GET /api/v1/analytics/student/departments
 */
export const getStudentDepartmentReports = asyncHandler(async (req, res) => {
  if (req.user.role !== "student") {
    throw ApiError.forbidden("Only students can access student reports");
  }
  ...
  if (isExport === "true") {
    const buffer = await ExportService.generateStudentReport("departments", result, "xlsx");
    res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
    res.setHeader("Content-Disposition", `attachment; filename=student_departments_report.xlsx`);
    return res.status(200).send(buffer);
  }

  res.status(200).json(
    new ApiResponse(200, { departments: result }, "Student department reports retrieved"),
  );
});

/**
 * Student Batch Reports
 * GET /api/v1/analytics/student/batches
 */
export const getStudentBatchReports = asyncHandler(async (req, res) => {
  if (req.user.role !== "student") {
    throw ApiError.forbidden("Only students can access student reports");
  }
  ...
  if (isExport === "true") {
    const buffer = await ExportService.generateStudentReport("batches", result, "xlsx");
    res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
    res.setHeader("Content-Disposition", `attachment; filename=student_batches_report.xlsx`);
    return res.status(200).send(buffer);
  }

  res.status(200).json(
    new ApiResponse(200, { batches: result }, "Student batch reports retrieved"),
  );
});

/**
 * Student Semester Reports
 * GET /api/v1/analytics/student/semesters
 */
export const getStudentSemesterReports = asyncHandler(async (req, res) => {
  if (req.user.role !== "student") {
    throw ApiError.forbidden("Not authorized to access reports");
  }
  ...
  if (isExport === "true") {
    const buffer = await ExportService.generateStudentReport("semesters", result, "xlsx");
    res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
    res.setHeader("Content-Disposition", `attachment; filename=student_semesters_report.xlsx`);
    return res.status(200).send(buffer);
  }

  res.status(200).json(
    new ApiResponse(200, { semesters: result }, "Student semester reports retrieved"),
  );
});

/**
 * Student Section Reports
 * GET /api/v1/analytics/student/sections
 */
export const getStudentSectionReports = asyncHandler(async (req, res) => {
  if (req.user.role !== "student" && req.user.role !== "teacher") {
    throw ApiError.forbidden("Not authorized to access reports");
  }
  ...
  if (isExport === "true") {
    const buffer = await ExportService.generateStudentReport("sections", result, "xlsx");
    res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
    res.setHeader("Content-Disposition", `attachment; filename=student_sections_report.xlsx`);
    return res.status(200).send(buffer);
  }

  res.status(200).json(
    new ApiResponse(200, { sections: result }, "Student section reports retrieved"),
  );
});

/**
 * Student Subject Reports
 * GET /api/v1/analytics/student/subjects
 */
export const getStudentSubjectReports = asyncHandler(async (req, res) => {
  if (req.user.role !== "student" && req.user.role !== "teacher") {
    throw ApiError.forbidden("Not authorized to access reports");
  }
  ...
  if (isExport === "true") {
    const buffer = await ExportService.generateStudentReport("subjects", result, "xlsx");
    res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
    res.setHeader("Content-Disposition", `attachment; filename=student_subjects_report.xlsx`);
    return res.status(200).send(buffer);
  }

  res.status(200).json(
    new ApiResponse(200, { subjects: result }, "Student subject reports retrieved"),
  );
});

/**
 * Student Class Reports
 * GET /api/v1/analytics/student/classes
 */
export const getStudentClassReports = asyncHandler(async (req, res) => {
  if (req.user.role !== "student" && req.user.role !== "teacher") {
    throw ApiError.forbidden("Not authorized to access reports");
  }
  ...
  if (isExport === "true") {
    ...
    const buffer = await ExportService.generateStudentReport("classes", result, "xlsx", formattedDetails);
    res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
    res.setHeader("Content-Disposition", `attachment; filename=student_classes_report.xlsx`);
    return res.status(200).send(buffer);
  }
  ...
  res.status(200).json(
    ...
    }, "Student class reports retrieved"),
  );
});

/**
 * Student Cross-Class Defaulters
 * GET /api/v1/analytics/student/defaulters
 * Returns students below threshold across all classes, with filters
 */
export const getStudentDefaulters = asyncHandler(async (req, res) => {
  if (req.user.role !== "student" && req.user.role !== "teacher") {
    throw ApiError.forbidden("Not authorized to access reports");
  }
  ...
  if (isExport === "true") {
    ...
    const buffer = await ExportService.generateStudentReport("defaulters", defaulters, "xlsx", formattedDetails);
    res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
    res.setHeader("Content-Disposition", `attachment; filename=student_defaulters_report.xlsx`);
    return res.status(200).send(buffer);
  }
 ...
 * GET /api/v1/analytics/student/students/:studentId/attendance
...
export const getStudentAttendanceDetail = asyncHandler(async (req, res) => {
  if (req.user.role !== "student" && req.user.role !== "teacher") {
    throw ApiError.forbidden("Only students and teachers can access this");
  }
...
/**
 * Student Class Attendance Detail
 * GET /api/v1/analytics/student/classes/:classId/attendance
 * Returns per-session attendance records for drill-down modal
 */
